...
function GnaniWebVoice(options) {
  ...
  self.connect    = function () {
    if (ws || self.isConnected) return;     // already connecting/connected
    isCleanedUp = false;                    // clear stale post-disconnect flag
    openConnection();
  };
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ── Bot configuration ─────────────────────────────────────────── */
  // The WebSocket URL only varies by bot ID; everything else stays constant.
  // If a bot ever needs its own api_key, change the BOTS entries to objects:
  //   { botId: '...', apiKey: '...' }  and adjust buildWsUrl().
  var WS_HOST    = 'wss://weborc.inya.ai/ws_speech_transcript/';
  var WS_API_KEY = '89b7952e-796a-46f4-b18b-3f67434e58f7';

  // Maps the <select> option values → bot IDs.
  // Values must match the <option value="..."> strings exactly.
  var BOTS = {
    'English & Hindi'         : 'afc92606fa2a462c905920738c3af529',
    'English & Tamil'         : '0df15e4d8a0548129eaa55d622c956f3',
    'US English & Spanish'    : 'c266c1af148d44e6b3a4be481b1ee4ce',
  };

  function buildWsUrl(botId) {
    var params = [
      'api_key='        + WS_API_KEY,
      'mode=test',
      'intent_id=flow123',
      'conversation_id=d2ee6a45-ba0a-4cc8-a987-24a44dccd2ed',
      'phone_number=0000000000',
      'organization_id=0e56a0b7-a89b-4076-bf88-d2fece19e5a0',
      'environment=production',
      'language=en-US',
    ].join('&');
    return WS_HOST + botId + '?' + params;
  }
  ...
  function endConversation() {
    ...
    cancelAgentTimer();
    enableLangSelect();
    clearOurBubbles();      // wipe the transcript so the next call starts fresh
    hidePlaceholders();     // keep placeholders hidden after hangup too
  }
  ...
  // Initialise with the first available bot — the URL is overwritten
  // every time the user clicks Call (based on the language dropdown).
  var defaultLang = Object.keys(BOTS)[0];
  var voice = new GnaniWebVoice({
    websocketUrl       : buildWsUrl(BOTS[defaultLang]),
    onOpen             : function () {
  ...
  vaTakeBtn.addEventListener('click', function () {
    // Pick the bot based on the language dropdown selection
    var selectedLang = langSelect ? langSelect.value : defaultLang;
    var botId        = BOTS[selectedLang];

    if (!botId) {
      console.warn('[VA] No bot mapped for language:', selectedLang, '— falling back to default');
      botId = BOTS[defaultLang];
    }

    var wsUrl = buildWsUrl(botId);
    console.log('[VA] Calling with bot:', selectedLang, '→', botId);
    voice.setWebsocketUrl(wsUrl);
    voice.connect();
  });
